...
export default function CheckpointPage({
  ...
  useEffect(() => {
    fetch(`/api/courses/${id}/checkpoints/${num}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleNext = async (e?: React.FormEvent) => {
    ...
    // Submit attempt to practice engine
    try {
      const res = await fetch(`/api/courses/${id}/practice`, {
        ...
        body: JSON.stringify({
          ...
          user_answer: answer,
          canonical_answer: currentItem.canonical_answer,
          exercise_type: 'checkpoint_test',
        }),
      });
  ...
  if (isFinished) {
    return (
      <FocusLayout exitHref={`/courses/${id}`} title="Výsledek Checkpointu" progressPercent={100}>
  ...
  return (
    <FocusLayout
      exitHref={`/courses/${id}`}
      title={`${checkpointData.title} (${currentIndex + 1}/${items.length})`}
