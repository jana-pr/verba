  ...
  return (
    <AppLayout activeCourseId={id}>
      <div className="space-y-6">
        {/* Course Header */}
        <div className="verba-card p-6 sm:p-7 border-indigo-100 bg-gradient-to-br from-white via-white to-indigo-50/20 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-md bg-indigo-50 text-verba-indigo font-bold text-xs border border-indigo-100">
                {course.target_language.toUpperCase()} • {course.cefr_level}
              </span>
              <span className="text-xs font-semibold text-verba-slate">
                Svobodný výběr lekcí
              </span>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-verba-ink tracking-tight">
              {course.domain_area}
            </h1>
            <p className="text-xs sm:text-sm text-verba-slate max-w-xl">
          ...
          <div className="flex flex-col sm:flex-row md:flex-col gap-2 shrink-0">
            <Link
              ...
              className="py-3 px-5 rounded-xl bg-verba-indigo hover:bg-verba-indigo-dark text-white font-semibold text-xs sm:text-sm shadow-md transition-all flex items-center justify-center gap-2"
            >
            ...
            <Link
              ...
              className="py-2.5 px-4 rounded-xl border border-slate-200 hover:border-indigo-200 bg-white text-verba-ink hover:text-verba-indigo font-medium text-xs transition-colors flex items-center justify-center gap-2"
            >
              <RotateCw className="w-3.5 h-3.5" />
              <span>Course Review (50/25/25)</span>
            </Link>
          </div>
        </div>

        {/* Milestone Checkpoints Info Bar */}
        <div className="grid grid-cols-5 gap-2 text-center select-none">
          {[10, 20, 30, 40, 50].map((cp) => (
            <Link
              key={cp}
              href={`/courses/${id}/checkpoints/${cp}`}
              className="p-2.5 rounded-xl bg-white border border-slate-200 hover:border-amber-300 hover:bg-amber-50/30 transition-all group"
            >
              <div className="text-[10px] font-bold text-verba-slate group-hover:text-verba-review uppercase">
                Checkpoint {cp / 10}
              </div>
              <div className="text-xs font-extrabold text-verba-ink mt-0.5">
                Lekce {cp}
              </div>
            </Link>
          ))}
