...
import { getCourseDetail, deleteCoursePermanently } from '@/lib/data-repository';
...
function CourseDetailContent() {
  ...
  const handleDeleteCourse = async () => {
    ...
    setIsDeleting(true);
    await deleteCoursePermanently(id);
    ...
    return (
      <AppLayout>
        <div className="text-center py-20 text-verba-slate">
          <p className="font-semibold text-base mb-2">Kurz nebyl nalezen.</p>
      ...
      <div className="space-y-6">
        ...
        <div className="flex gap-1.5 sm:gap-2 text-center select-none overflow-x-auto pb-1 no-scrollbar w-full max-w-full">
          {[10, 20, 30, 40, 50].map((cp) => (
            <Link
              key={cp}
              href={`/courses/checkpoints?id=${id}&num=${cp}`}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {lessonItems.map((item) => (
              <Link
                key={item.number}
                href={`/courses/lessons?id=${id}&lessonId=${item.number}`}
...
export default function CourseDetailPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20 text-verba-slate">Načítání kurzu...</div>}>
      <CourseDetailContent />
    </Suspense>
  );
}
