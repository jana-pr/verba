...
import { getLessonDetail } from '@/lib/data-repository';
...
function LessonDetailContent() {
    ...
    return (
      <AppLayout activeCourseId={id}>
        <div className="text-center py-20 text-verba-slate">
          <p className="font-semibold text-base mb-2">Lekce nebyla nalezena.</p>
          <Link href={`/courses/view?id=${id}`} className="text-xs font-semibold text-verba-indigo hover:underline">
  ...
  const currentNum = Number(lesson.lesson_number) || 1;
  ...
  const handleCheckExercise = async (ex: any) => {
    ...
    try {
      const evaluation = await evaluateAnswer(answer, ex.canonical_answer, ex.acceptable_synonyms || []);
      setExerciseResults((prev) => ({ ...prev, [ex.id]: evaluation }));
    } catch (e) {
      console.error(e);
      const isCorrect = answer.trim().toLowerCase() === (ex.canonical_answer || '').trim().toLowerCase();
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-200/60 pb-4">
          ...
          <div className="flex items-center gap-2">
            <Link
              href={`/courses/practice?id=${id}&lessonId=${lesson.lesson_number}`}
            ...
            {currentNum < 50 && (
              <Link
                href={`/courses/lessons?id=${id}&lessonId=${currentNum + 1}`}
                className="p-2 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 text-verba-slate transition-colors"
                title="Další lekce"
              >
        ...
        {/* TAB 1: VOCABULARY & PHRASES (Contextual cards with bidirectional states) */}
        {activeTab === 'vocab' && (
          <div className="space-y-3">
            <div className="flex items-center justify-between px-1 text-xs text-verba-slate font-medium">
              <span>{items?.length || 0} odborných výrazů a frází</span>
              ...
              {items?.map((item: any) => (
                ...
                >
                  ...
                  <div className="flex md:flex-col items-center md:items-end justify-between gap-2 border-t md:border-t-0 pt-2 md:pt-0 shrink-0">
                    ...
                    <Link
                      href={`/courses/practice?id=${id}&lessonId=${lesson.lesson_number}&direction=cz_to_target`}
        ...
        {activeTab === 'listening' && (
          <div className="verba-card p-6 sm:p-8 space-y-6 max-w-3xl mx-auto">
            ...
            <div className="pt-2">
              <Link
                href={`/courses/practice?id=${id}&lessonId=${lesson.lesson_number}&type=sentences`}
...
export default function LessonDetailPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20 text-verba-slate">Načítání lekce...</div>}>
      <LessonDetailContent />
    </Suspense>
  );
}
